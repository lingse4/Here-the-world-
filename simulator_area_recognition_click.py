...
config_path = os.path.join(os.path.dirname(__file__), 'fzbconfig.json')
def load_device_from_config():
    with open(config_path, 'r', encoding='utf-8') as f:
            config = json.load(f)
    return config.get('device_serial')

# -------------------- 1. 初始化ADB连接 --------------------
def check_adb_connection(device_serial=load_device_from_config()):
    """检查ADB设备是否已连接"""
...
# -------------------- 2. ADB截图函数 --------------------
def adb_screenshot(save_path, device_serial=load_device_from_config()):
    """截取模拟器屏幕并保存为文件"""
    adb_path = r"D:\BaiduNetdiskDownload\platform-tools-latest-windows\platform-tools\adb"  # ADB 的完整路径
...
# -------------------- 5. ADB点击函数 --------------------
def adb_tap(x, y, device_serial=load_device_from_config()):
    """使用ADB命令在指定坐标点击"""
    adb_path = r"D:\BaiduNetdiskDownload\platform-tools-latest-windows\platform-tools\adb"  # ADB 的完整路径
...
def routineo(img_model_path, name, threshold=0.8, region=None):
...
#adb_tap(1085,500)
#avg = routineo(r"C:\Users\38384\Pictures\Screenshots\代理.png", "l", threshold=0.4, region=(900,400,400,400))
