...
def get_xy(img_model_path, threshold=0.8, region=None):
    ...
    if len(img.shape) == 3 and img.shape[2] == 4:
        img = cv2.cvtColor(img, cv2.COLOR_BGRA2BGR)  # 移除Alpha通道
    elif len(img.shape) == 2:
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)  # 灰度转BGR
    
    # 处理模板图像
    if len(img_terminal.shape) == 3 and img_terminal.shape[2] == 4:
        img_terminal = cv2.cvtColor(img_terminal, cv2.COLOR_BGRA2BGR)  # 移除Alpha通道
    elif len(img_terminal.shape) == 2:
        img_terminal = cv2.cvtColor(img_terminal, cv2.COLOR_GRAY2BGR)  # 灰度转BGR
...
#adb_tap(1085,500)
avg = routine(r"C:\Users\Administrator\Desktop\Matcha Parfait\Pictures\Screenshots\4.png", "l", threshold=0.8)
