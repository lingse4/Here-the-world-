...
def Clear_levels(name):
    ...
    while True:
        ...
        if onebs_coords:
            mini_record(r"C:\Users\38384\Desktop\抹茶巴菲\mouse_operations.txt")
            break
...
def process_stage(SCREENSHOT_MAIN, max_retries):
    ...
    for attempt in range(max_retries):
        try:
            ...
            coords1 = get_xy(TEMPLATE_PATHS['unplayed'], SCREENSHOT_MAIN, threshold=0.7)
            ...
            if coords3:
                ...
                # 新增：启动mini_record录制线程（每20秒执行一次）
                record_running = True  # 线程停止标志
                
                def record_loop():
                    while record_running:
                        mini_record(r"C:\Users\38384\Desktop\抹茶巴菲\mouse_operations.txt")
                        time.sleep(20)  # 每20秒执行一次录制
                
                record_thread = threading.Thread(target=record_loop, daemon=True)
                record_thread.start()
                
                # 持续检测operation图片，检测到后终止录制
                print("开始检测operation图片...")
                if wait_for_image(TEMPLATE_PATHS['operation'], SCREENSHOT_MAIN):
                    record_running = False  # 触发停止标志
                    record_thread.join()    # 等待线程安全退出
