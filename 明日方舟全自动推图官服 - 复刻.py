...
def process_stage(SCREENSHOT_MAIN, max_retries):
    ...
    for attempt in range(max_retries):
        try:
            ...
            coords4 = get_xy(TEMPLATE_PATHS['ys1'], SCREENSHOT_MAIN, threshold=0.6)
            coords5 = get_xy(TEMPLATE_PATHS['xd'], SCREENSHOT_MAIN, threshold=0.7)
            ...
            if coords:
                ...
                if txms_coords:
                    print("有突袭")
                    if routineo(r"C:\Users\38384\Pictures\Screenshots\代理.png", "l", threshold=0.9, region=(900,400,400,400)):
                        print("有代理")
                        dj(*START_COORDS)
                        adb_screenshot(SCREENSHOT_MAIN)  # 全屏截图 
                        start_coords = (1245,661)
        ...
        for item1 in new_results:
            ...
            if result == "特殊关卡":
                ...
                for item1 in new_results:
                    ...
                    print(f"识别到文本: {text} (置信度: {confidence:.2f}) 坐标: ({x_center}, {y_center})")
                    print(item1)
                    if re.search(r'-8$', text):  # 匹配以"-8"结尾的字符串
                        print(f"{text} 符合条件")
                        dj(x_center, y_center)
                        ...
                        txms_coords=r"C:\Users\38384\Pictures\Screenshots\txms.png"
                        START_COORDS = (1245, 661)
