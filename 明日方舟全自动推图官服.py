...
# 修改commands列表，使用adb_path变量
commands = [
    f"{adb_path} connect 127.0.0.1:16384",
    f"{adb_path} -s 127.0.0.1:16384 shell am start-foreground-service --user 0 -a jp.co.cyberagent.stf.ACTION_START -n jp.co.cyberagent.stf/.Service && {adb_path} -s 127.0.0.1:16384 forward tcp:1100 localabstract:stfservice",
    f"{adb_path} -s 127.0.0.1:16384 forward tcp:1090 localabstract:stfagent && {adb_path} -s 127.0.0.1:16384 shell pm path jp.co.cyberagent.stf && {adb_path} -s 127.0.0.1:16384 shell export CLASSPATH=\"package:/data/app/~~Jr-GxeGnZxBjP1TWUJA3rw==/jp.co.cyberagent.stf-yTMn6Do7T6PDkxk1PiTaBg==/base.apk\";exec app_process /system/bin jp.co.cyberagent.stf.Agent",
    f"{adb_path} -s 127.0.0.1:16384 shell chmod 755 /data/local/tmp/minitouch && {adb_path} -s 127.0.0.1:16384 shell /data/local/tmp/minitouch",
    f"{adb_path} -s 127.0.0.1:16384 forward tcp:1090 localabstract:minitouch"
]

# 定义CREATE_NO_WINDOW标志
...
SCREENSHOT_DIR = r"C:\Users\Administrator\Pictures\Screenshots"
...
# 初始化 EasyOCR 读取器
ocr = PaddleOCR(use_angle_cls=True, lang='en')  # 这里可以选择不同的语言
...
def get_xy(img_model_path, SCREENSHOT_MAIN, threshold=0.8):
    ...
    template_gray = cv2.cvtColor(img_terminal, cv2.COLOR_RGB2GRAY)
...
def Clear_levels(name):
    ...
    while True:
        ...
        if onebs_coords:
            mini_record(r"C:\Users\Administrator\Desktop\抹茶巴菲\mouse_operations.txt",PORT)
            break
...
def process_stage(SCREENSHOT_MAIN, max_retries):
    ...
    # 图像模板路径配置
    TEMPLATE_PATHS = {
        'yx': r"C:\Users\38384\Pictures\Screenshots\yx.png",
        'ys': r"C:\Users\38384\Pictures\Screenshots\ys.png",
        'txms': r"C:\Users\38384\Pictures\Screenshots\txms.png",
        'ksjq': r"C:\Users\38384\Pictures\Screenshots\ksjq.png",
        'jzyx': r"C:\Users\38384\Pictures\Screenshots\jzyx.png",
        'ys1': r"C:\Users\38384\Pictures\Screenshots\ys1.png",
        'strategy': r"C:\Users\38384\Pictures\Screenshots\bsgy.png",
        'xd': r"C:\Users\38384\Pictures\Screenshots\xd.png",
        '代理': r"C:\Users\38384\Pictures\Screenshots\代理.png",
    }
    ...
    for attempt in range(max_retries):
        try:
            ...
            if coords3:
                print("[流程] 检测到演习关卡")
                if routineo(r"C:\Users\38384\Pictures\Screenshots\代理.png", "代理", threshold=0.9, region=(900,400,400,400)):
                    print("有代理")
                    dj(*START_COORDS)
                    adb_screenshot(SCREENSHOT_MAIN)  # 全屏截图
                    start_coords = (1245,661)
...
# 合并匹配规则：同时匹配 13-14 和 TR-15 格式
    ...
    for item2 in matched_texts:
        ...
        for item1 in matched_texts0:

            if item1[0] in processed_texts:
                continue
            match = next((r for r in item1[0] if r == item2[0]), None)
            if match is None:
            ...
            # 计算 x 坐标的平均值
            x_center = 1280-sum([new_bbox[0] + new_bbox[2]])/2

            # 计算 y 坐标的平均值
            y_center = 720-sum([new_bbox[1] + new_bbox[3]])/2


            print(f"识别到文本: {item2[0]},坐标: ({x_center}, {y_center})")
